    ...
    def get_object(self, queryset=None):
        ...
        try:
            if 'pk' in self.kwargs:
                ...
                raise Http404(_("Impossible de trouver le paramètre 'pk'."))
        except ValueError as badvalue:
            raise Http404(_("La valeur du paramètre pk '{}' n'est pas un entier valide.".format(badvalue)))
        ...
        if not obj:
            raise Http404(_("Aucun contenu ne possède cet identifiant."))
        ...
        if 'slug' in self.kwargs:
            ...
            if versioned.slug != slug:
                if slug != self.object.slug:  # retro-compatibility, but should raise permanent redirect instead
                    raise Http404(_("Ce slug n'existe pas pour ce contenu."))
...
class SingleOnlineContentViewMixin(ContentTypeMixin):
    ...
    def get_public_object(self):
        ...
        if obj is None:
            raise Http404(_("Aucun contenu ne possède ce slug."))
        ...
        if obj.must_redirect:
            if obj.content.public_version and self.redirection_is_needed:
                ...
                raise Http404(_("La redirection est activée mais le contenu n'est pas public."))
    ...
    def get_object(self):
        ...
        if obj is None:
            raise Http404(_("Le contenu de la publication n'est pas trouvé."))
        return obj

...
class ContainerFactory(factory.Factory):
    ...
    def _prepare(cls, create, **kwargs):
        ...
        f = open(os.path.join(container.top_container().get_path(), container.introduction), "w")
        f.write(text_content.encode('utf-8'))
        f.close()
        repo.index.add([container.introduction])
        f = open(os.path.join(container.top_container().get_path(), container.conclusion), "w")
        f.write(text_content.encode('utf-8'))
        f.close()
        repo.index.add([container.conclusion])

        container.top_container().dump_json()

...
class RedirectOldBetaTuto(RedirectView):
    ...
    def get_redirect_url(self, **kwargs):
        ...
        if tutorial is None or tutorial.sha_beta is None or tutorial.sha_beta == "":
            raise Http404(_("Aucun contenu en bêta trouvé avec cet ancien identifiant."))
...
class DisplayBetaContent(DisplayContent):
    ...
    def get_object(self, queryset=None):
        ...
        if not obj.sha_beta or obj.sha_beta == '':
            raise Http404(_("Aucune bêta n'existe pour ce contenu."))
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if 'object_slug' in self.kwargs:
            try:
                ...
                raise Http404(_("Impossible de récupérer le contenu pour le supprimer."))
...
class DisplayDiff(LoggedWithReadWriteHability, SingleContentDetailViewMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'from' not in self.request.GET:
            raise Http404(_("Paramètre GET 'from' manquant."))
...
class WarnTypo(SingleContentFormViewMixin):
    ...
    def form_valid(self, form):
        ...
        if form.content.is_public:
            ...
            raise Http404(_("Le contenu n'est ni public, ni en bêta."))
...
class MoveChild(LoginRequiredMixin, SingleContentPostMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if base_container_slug == '':
            raise Http404(_("Le slug du container de base est vide."))

        if child_slug == '':
            raise Http404(_("Le slug du container enfant est vide."))
        ...
        try:
                ...
                if not parent.has_child_with_path(target):
                    if "/" not in target:
                        ...
                        if target.split("/")[-1] not in target_parent.children_dict:
                            raise Http404(_("La cible n'est pas un enfant du parent."))
            ...
            raise Http404(_("L'arbre spécifié n'est pas valide." + str(e)))
        except IndexError:
...
class ContentOfAuthor(ZdSPagingListView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.user != self.request.user and 'filter' in self.request.GET:
            ...
            if filter_ in self.authorized_filters:
                ...
                raise Http404(_("Le filtre n'est pas autorisé."))
    ...
    def get_queryset(self):
        if self.type in list(TYPE_CHOICES_DICT.keys()):
            ...
            raise Http404(_("Ce type de contenu est inconnu dans le système."))
